/**
 * MCP Server factory - creates and configures the Harbor MCP server
 * This logic is shared between stdio and HTTP transports
 */
...
export interface ServerContext {
  ...
  bidPollingService: BidPollingService;
  deliveryPollingService: DeliveryPollingService;
}

/**
 * Initialize Harbor client and services
 */
export async function initializeServerContext(): Promise<ServerContext> {
...
export function createMcpServer(context: ServerContext): Server {
  const { harborClient, bidPollingService, deliveryPollingService } = context;
